...
describe("Test People Controller", () => {
  ...
  it("should return 200 response and fetch specified data", async () => {
    ...
    expect(res.body.success).toBe(true);
    expect(res.body.data[0].name).toBe("Ackbar");
    expect(res.body.data[0].height).toBe("180");
    expect(typeof res.body).toEqual("object");
});
...
it("should return 200 response and fetch data of page 2", async () => {
  ...
  expect(res.body.data[0].name).toBe("Bib Fortuna");
  expect(res.body.data[0].films[0].title).toBe("Return of the Jedi");
  expect(typeof res.body).toEqual("object");
});
